...
<script type="text/javascript">
	$(function(){
		...
		$("#searchInput").keydown(function (event) {
			if (event.keyCode==13){
				$.ajax({
					url:"workbench/clue/findActivityByName.do",
					data:{
		...
		$("#convertBut").click(function () {
			...
			if ($("#isCreateTransaction").prop("checked")) {
				...
				window.location.href="workbench/clue/convert.do?flag=b&clueId=${xx.id}"
			}
		})
	});
</script>
...
<body>
					...
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input type="text" class="form-control" style="width: 300px;" id="searchInput" placeholder="请输入市场活动名称，支持模糊查询">
	...
	<div id="title" class="page-header" style="position: relative; left: 20px;">
		<h4>转换线索 <small>${xx.fullname}${xx.appellation}-${xx.company}</small></h4>
	</div>
	<div id="create-customer" style="position: relative; left: 40px; height: 35px;">
		新建客户：${xx.company}
	</div>
	<div id="create-contact" style="position: relative; left: 40px; height: 35px;">
		新建联系人：${xx.fullname}${xx.appellation}
	</div>
	...
	<div id="create-transaction2" style="position: relative; left: 40px; top: 20px; width: 80%; background-color: #F7F7F7; display: none;" >
		...
		<form action="workbench/clue/convert.do" method="post" id="convertForm">
			<input name="clueId" type="hidden" value="${xx.id}">
			<input name="flag" value="a" type="hidden">
	...
	<div id="operation" style="position: relative; left: 40px; height: 35px; top: 100px;">
		<input class="btn btn-primary" type="button" id="convertBut" value="转换">
		&nbsp;&nbsp;&nbsp;&nbsp;
